---
description: Bulk-review multiple legal documents by extracting structured data into an Excel spreadsheet — each document a row, each review question a column (Polish law context with IURA MCP)
...
Masowy przegląd wielu dokumentów prawnych (umowy, NDA, umowy najmu, umowy kredytowe, SPA, MSA itp.) przez ekstrakcję ustrukturyzowanych danych do arkusza Excel. Każdy dokument to wiersz, każde pytanie przeglądowe to kolumna, a każda komórka zawiera wyekstrahowaną odpowiedź z odniesieniem do źródła — wzbogacona o aktualne orzecznictwo i przepisy prawa polskiego dzięki IURA MCP.
...
### Krok 4: Weryfikacja prawna z IURA MCP

Przed rozpoczęciem ekstrakcji, zweryfikuj stan prawny:

1. **Przepisy prawa** (Search_DU): Sprawdź aktualne brzmienie przepisów właściwych dla typu przeglądanych dokumentów z parametrem `in_force_as_of` ustawionym na dzisiejszą datę.

2. **Przepisy bezwzględnie obowiązujące** (Search_DU): Zweryfikuj przepisy ius cogens (art. 58 KC, art. 473 § 2 KC, art. 483 § 1 KC) dla oznaczania klauzul potencjalnie nieważnych.

3. **Najnowsze zmiany** (Search_News): Sprawdź bieżące zmiany prawne mogące wpływać na przeglądane dokumenty.
...
- **RED**: Poza akceptowalnym zakresem; wymaga eskalacji — obowiązkowo przeszukaj IURA (Search_SN, Search_SP) dla wsparcia klasyfikacji orzecznictwem
...
- Legenda kolorowania
- Kluczowe ustalenia IURA (najistotniejsze przepisy i orzeczenia)
...
- Sygnatury orzeczeń i przepisy dla kolumn wspieranych przez IURA
...
3. **Weryfikacja IURA**: Sprawdź aktualność powołanych przepisów i sygnatur
...
### Kluczowe przepisy i orzeczenia

[Najistotniejsze przepisy i sygnatury przywołane w analizie]
...
- Zidentyfikowane ryzyka można formalnie ocenić przy użyciu skilla `legal-risk-assessment`

## Uwagi

- Dla zbiorów zawierających różne typy dokumentów, zaproponuj osobne arkusze per typ lub nadrzędny zestaw kolumn z „N/D"
- Jeśli playbook jest skonfigurowany w `legal.local.md`, użyj jego progów ryzyka; w przeciwnym razie stosuj ogólne standardy rynkowe
- Zawsze przypominaj, że wyekstrahowane dane powinny być zweryfikowane przez wykwalifikowanego prawnika
- Powołania na przepisy prawa i orzecznictwo (z IURA MCP) podawaj z sygnaturami dla łatwej weryfikacji
- Żadna klasyfikacja RED nie powinna być przedstawiana bez uprzedniego przeszukania baz IURA pod kątem aktualnego orzecznictwa i stanu prawnego
